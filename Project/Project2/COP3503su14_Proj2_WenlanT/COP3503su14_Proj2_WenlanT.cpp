	...
	int MultiSet::findItem(string itemName){
		...
		while(getline(cin,input)){
			...
			if (!record.eof()){
				...
				cout << "please reenter the <item name>" << endl;
				continue;
			}
		...
		while(getline(cin,input)){
			...
			if (record.fail()) {
				if (input.empty()){
					if (!silent){
						cerr << "No input!" << endl;
						cout << "Please enter the <item name> and <count> to be inserted" << endl;
					}
				} 
				...
				if (input==itemName){
					...
					if(!silent){
					cerr << "[" << input << "] is not in correct format on user input line" << endl;
					cout << "Please make sure the correct format: < item name> <count>" << endl;
					}				
				}	
				continue;
			}
