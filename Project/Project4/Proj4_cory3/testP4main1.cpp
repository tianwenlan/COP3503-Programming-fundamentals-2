  ...
  while(run)
  {
    ...
    if (cin.good()) 
    {
      switch (operation_num)
      {
    	...
    	case set_difference1://difference
    	{
		MySet<T> otherSet;
      		if (readSet(otherSet))
		{
    	...
    	case set_less_than: // contained in
	{
    	...
    	case set_subtract2://substract   -=  21
    	{
		MySet<T> otherSet;
      		if (readSet(otherSet))
		{
    	...
    	case set_difference2://difference   ^=  22
    	{
		MySet<T> otherSet;
      		if (readSet(otherSet))
		{
    	...
    	case set_intersect2://intersect  *=   23
    	{
		MySet<T> otherSet;
      		if (readSet(otherSet))
		{
