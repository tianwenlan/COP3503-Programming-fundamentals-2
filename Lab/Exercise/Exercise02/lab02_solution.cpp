    ...
    Graph(){
	  ...
	  // Print the entire graph according to the format in the
    	...
    	void printEdgesOfVertex(string vert){
			...
			if (p==-1) {cout << "Error: [" << vert << "] does not exist" << endl; return;}
			cout << "[" << vert << "]:";
			for (int i=0; i<numEdges; i++)
   	 ...
   	 void addEdge(string vert1, string vert2){
		 ...
		 if (p==-1) {cout << "Error: [" << vert1 << "] does not exist" << endl; return;}
		 if (q==-1) {cout << "Error: [" << vert2 << "] does not exist" << endl; return;}
		 if (p==q) {cout << "Error: Edge must connect two different vertices"<<endl; return;}
...
int main(int argc, char ** argv){
	...
	cout << ">";
		...
		switch (option){
		    ...
		    cin >> vertIn;
		    //if (vertIn != vert){
			//	cout << "Error: [" << vertIn << "] does not exist" << endl;
			//	cout << '\n' << endl;
			//}
			//else if (vertIn == vert) {				
